...
Module Program
    ...
    Sub Main(args As String())
        ...
        'Dim cmdAcciones2 As New SqlCommand("SELECT A.IdAccion, A.IdExpediente, TN.idTipoNota, A.Fecha, RL.IdTipoAccion FROM Acciones AS A JOIN Expedientes AS E ON A.IdExpediente = E.IdExpediente LEFT JOIN ResultadoLlamada AS RL ON A.IdAccion = RL.IdAccion LEFT JOIN TipoNota AS TN ON A.IdTipoNota = TN.idTipoNota WHERE CONVERT(DATE,A.Fecha) >= '20/05/2024' AND CONVERT(DATE,A.Fecha) < '31/05/2024' AND E.idCliente=91 AND A.idTipoNota IN (5, 7, 8, 9, 10, 11, 13, 14, 15, 22, 79, 80, 81, 656, 742, 744, 746, 788, 789, 820, 821, 822, 823, 824, 825, 826, 827, 828, 831, 832, 834, 836, 837, 840, 849, 854, 861, 915, 917, 918, 921, 922) ORDER BY A.Fecha ASC", connection)
